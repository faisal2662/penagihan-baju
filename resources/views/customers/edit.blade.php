            ...
            <div class="col col-lg-8 col-md-10">
                <div class="card">
                    <div class="card-body">
                        ...
                        @endif
                        <form action="/edit-customer/{{ $customer->slug }}" method="post">
                            @csrf
                                ...
                                <select name="size" id="size" class="form-select">
                                    ...
                                    @foreach ($price as $item)
                                        <option value="{{ $item->slug }}"
                                            {{ $item->id == $customer->slug_price_list ? 'selected' : '' }}>
                                            {{ $item->size }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="color" id="color" class="form-select">
                                    ...
                                    @foreach ($color as $item)
                                        <option value="{{ $item->slug }}"
                                            {{ $item->slug == $customer->slug_color ? 'selected' : '' }}>
                                            {{ $item->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="category" id="category" class="form-select">
                                    ...
                                    @foreach ($category as $item)
                                        <option value="{{ $item->slug }}"
                                            {{ $item->slug == $customer->slug_category ? 'selected' : '' }}>
                                            {{ $item->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            @if ($sess != null)
                                <div class="mb-3">
                                    <label for="session" class="form-label">Jenis</label>
                                    <select name="session" id="session" class="form-select">
                                        <option value="" selected disabled>-- Pilih Jenis ---</option>
                                        @foreach ($sess as $item)
                                            <option value="{{ $item->slug }}"
                                                {{ $item->slug == $customer->slug_session ? 'selected' : '' }}>
                                                {{ $item->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="status" id="status" class="form-select">
                                    <option value="Belum Lunas" {{ $customer->status == 'Belum Lunas' ? 'selected' : '' }}>
                                        Belum</option>
                                    <option value="Lunas" {{ $customer->lunas == 'lunas' ? 'selected' : '' }}>Lunas
                                    </option>
                                </select>
                            </div>
                            <div class="mb-3">
                                <label for="Total" class="form-label">Total</label>
                                <input type="text" id="totalTemp" value="{{ $customer->payment->total }}"
                                    class="form-control" readonly>
                                <input type="hidden" name="total" value="{{ $customer->payment->total }}" id="total"
                                    class="form-control" readonly>
                            </div>
